...
function trivTimerLoop( timerVar, durInSec, onDone, updatefunc, propsStr ){
	var timerVarVal = timerVar.getValue();
	var startTime = parseInt( timerVarVal );
	var paused = false;
	var now = parseInt(new Date().getTime()/1000)*1000;
	if( typeof(timerVarVal) == "string" && timerVarVal.indexOf( "pause:" ) != -1 )
	{
		paused = true;
		var remainingTime = parseInt( timerVarVal.split(':')[1]) ;
		startTime = ( now - remainingTime );
		//if the user paused, just get a new starttime that accounts for the remaining time.
	}
	...
	var strExec = "trivTimerLoop(" + timerVar.name + "," + durInSec + ",'" + onDone +"','" + updatefunc + "', '" + propsStr + "' )";
